...
int main()
{
	...
	dynamic_cast<directory*>(root[0])->list_of_elements.push_back(new directory("master", dynamic_cast<directory*>(root[0])));
	//root[0]->list_of_elements.push_back(new directory("master", root[0]));
	dynamic_cast<directory*>(root[0])->list_of_elements.push_back(new file("project.cpp", dynamic_cast<directory*>(dynamic_cast<directory*>(root[0])->list_of_elements[0])));
	//root[0]->list_of_elements[0]->list_of_elements.push_back(new file("project.cpp", root[0]->list_of_elements[0]));
	dynamic_cast<directory*>(root[0])->list_of_elements.push_back(new file("readme.txt", dynamic_cast<directory*>(dynamic_cast<directory*>(root[0])->list_of_elements[0])));
	//root[0]->list_of_elements[0]->list_of_elements.push_back(new file("readme.txt", root[0]->list_of_elements[0]));
	dynamic_cast<directory*>(root[0])->list_of_elements.push_back(new directory("dev", dynamic_cast<directory*>(root[0])));
	//root[0]->list_of_elements.push_back(new directory("dev", root[0]));
	dynamic_cast<directory*>(root[0])->list_of_elements.push_back(new file("readme.txt", dynamic_cast<directory*>(dynamic_cast<directory*>(root[0])->list_of_elements[1])));
	...
	while (true)
	{
		...
		{
			print_tree(dynamic_cast<directory*>(tmp_root)->list_of_elements, 0);
			continue;
		}
		if (cmd == "exit")
		{
...
void print_tree(std::vector<element*> pwd, int depth)
{
	for (auto element : pwd)
	{
		...
		if (dynamic_cast<directory*>(element) != nullptr && dynamic_cast<directory*>(element)->list_of_elements.capacity() != 0)
		{
			depth++;
